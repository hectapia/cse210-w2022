...
class Director:
    ...
    def _do_outputs(self):
        ...
        if self._hider.is_found():                              # Enhanced game play.
            self._is_playing = (self._terminal_service.read_text("\n play again? [y]: ") == 'y')
            if self._is_playing:
                self._hider._location = random.randint(1, 1000) # Re-start _location
                self._hider._distance = [0, 0]                  # Re-start _distance
            else:
                self._terminal_service.write_text("\n Game Over ¨.¨ \n")
                self._is_playing = False                        # Game over messages.